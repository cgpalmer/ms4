...
class Order(models.Model):
    ...
    def _generate_order_number(self):
        """
        Generate a random, unique order number using UUID for the order number.
        """
    ...
    def update_total(self):
        ...
        if self.order_total < settings.FREE_DELIVERY_AMOUNT:
            self.delivery_cost = Decimal.from_float(settings.STANDARD_DELIVERY_AMOUNT)
        else:
...
class OrderLineItem(models.Model):
    ...
    lineitem_total = models.DecimalField(max_digits=6, default=0, decimal_places=2,
...
class Linked_Product(models.Model):
    ...
    linked_product = models.CharField(max_length=254, null=False, blank=True)
